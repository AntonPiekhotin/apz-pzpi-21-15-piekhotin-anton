...
function Users() {
    ...
    async function fetchUsers() {
        let response = await axios.get('https://localhost:8080/api/v1/auth/users', {headers: {Authorization: `Bearer ${localStorage.getItem('token')}`}});
    ...
    async function exportUsers() {
        try {
            await axios.get('https://localhost:8080/api/v1/auth/export-users', {headers: {Authorization: `Bearer ${localStorage.getItem('token')}`}});
    ...
    async function changeUser(user) {
        try {
            const response = await axios.post(`https://localhost:8080/api/v1/auth/edit-user/${user.id}`, user, {
                headers: {
