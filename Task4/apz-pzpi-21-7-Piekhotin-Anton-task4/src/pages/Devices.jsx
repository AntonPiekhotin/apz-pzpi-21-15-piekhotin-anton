...
function Devices() {
    ...
    async function fetchDevices() {
        let response = await axios.get('https://localhost:8080/api/v1/device/all', {headers: {Authorization: `Bearer ${localStorage.getItem('token')}`}});
    ...
    async function changeDevice(device) {
        try {
            const response = await axios.post(`https://localhost:8080/api/v1/device/edit/${device.id}`, device, {
                headers: {
